                        ...
                        async with httpx.AsyncClient(limits=limits) as client:
                            async with write_stream_reader:
                                async for session_message in write_stream_reader:
                                    ...
                                    try:
                                        logger.debug(f"Sending request to {post_url}: {json_payload.get('method', 'unknown')}")
                                        response = await client.post(
                                            post_url,
                                            json=json_payload,
                                            headers=request_headers,
                                            # Large read timeout for long tool executions
                                            timeout=httpx.Timeout(timeout, read=timeout, connect=60.0),
                                        )
                                        ...
                                        if "application/json" in content_type:
                                            try:
                                                data = response.json()
                                                logger.debug(f"Parsed direct JSON-RPC response: {data}")
                                                ...
                                                try:
                                                    ...
                                                    if isinstance(data, dict) and 'error' in data:
                                                        logger.warning(f"Validation failed for error response: {validation_exc}. Trying to patch ID.")
                                                    ...
                                                    is_notification = isinstance(json_payload, dict) and 'method' in json_payload and 'id' not in json_payload
                                                    
                                                    if is_notification:
                                                         logger.warning(f"Ignoring validation error for notification response: {validation_exc}")
                                                    else:
                                                        logger.error(f"Error validating JSON-RPC response: {validation_exc}. Data: {data}")
                                                        await read_stream_writer.send(validation_exc)

                                            except Exception as exc:
                                                logger.error(f"Error parsing direct JSON response: {exc}")
                                            ...
                                            for line in content.split('\n'):
                                                if line.startswith('data: '):
                                                    ...
                                                    try:
                                                        ...
                                                        logger.debug(f"Parsed SSE JSON-RPC response: {message}")
                                                        await read_stream_writer.send(SessionMessage(message))
                                                    except Exception as exc:
                                                        logger.error(f"Error parsing SSE response: {exc}. Data: {data}")
                                                        await read_stream_writer.send(exc)
                                                    break
